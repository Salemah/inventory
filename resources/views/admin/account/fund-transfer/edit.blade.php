...
@endif
    ...
    <form action="{{ route('admin.account.fund-transfer.update',$fundTransfer->id) }}" enctype="multipart/form-data" method="POST">
        @csrf
        @method('PUT')
        <div class="row">
            <div class="col-md-12">
                     ...
                     <div class="row">
                            <div class="form-group col-12 col-sm-12 col-md-6 mb-2">
                                 <label for="fund_transaction_title"><b>Fund Transfer Title</b><span class="text-danger">*</span></label>
                                    <input type="text" name="fund_transaction_title" id="fund_transaction_title"class="form-control" value="{{$transaction->transaction_title}}"placeholder="Enter Title...">
                            ...
                            <div class="form-group col-12 col-sm-12 col-md-6 mb-2">
                                ...
                                <input type="date" name="fund_transaction_date" id="fund_transaction_date"class="form-control" value="{{$transaction->transaction_date}}">
                                ...
                                <select name="form_account_id" id="form_account_id" class="form-control"onchange="getBalance()">
                                    <option value="" selected>-- Select Bank Account --</option>
                                     @foreach($bankAccounts as $bankAccount)
                                        <option value="{{ $bankAccount->id }}" @if ($bankAccount->id == $fundTransfer->cash_out_account) selected @endif>{{ $bankAccount->name }} | {{ $bankAccount->account_number }}
                                        @if ($bankAccount->type == 1) | Cash-Account @else | Bank-Account @endif</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group col-12 col-sm-12 col-md-6 mb-2">
                                ...
                                <select name="to_account_id" id="to_account_id" class="form-control">
                                    <option value="" selected>-- Select Bank Account --</option>
                                    @foreach($bankAccounts as $bankAccount)
                                        <option value="{{ $bankAccount->id }}" @if ($bankAccount->id == $fundTransfer->cash_in_account) selected @endif>{{ $bankAccount->name }}| {{ $bankAccount->account_number }}@if ($bankAccount->type == 1) | Cash-Account @else | Bank-Account @endif</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group col-12 col-sm-12 col-md-6 mb-2">
                                <label for="deposit_source"><b>Amount</b><span class="text-danger">*</span>
                                    ...
                                    <span class="text-info" id="balance" style="display: none">*</span><span  id="balanceHide">( <span class="text-info">{{ $accountBalance }}</span> )</span></label>
                                <input type="hidden" value="{{ $accountBalance }}" id="amount_balance">

                                <input type="number" min="0" name="amount" id="amount" class="form-control" value="{{ $fundTransfer->amount }}" placeholder="1200 ..." onkeyup="checkAmount(this)">
