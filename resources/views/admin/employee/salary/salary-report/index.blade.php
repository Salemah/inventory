...
@section('breadcrumb')
    <nav aria-label="breadcrumb" class="d-flex align-items-center justify-content-between" style="width: 100%">
        ...
        <a class="btn btn-sm btn-success text-white" href="{{ route('admin.salaryReport.create') }}">
            <i class='bx bx-plus'></i> Create Employee Salary
        </a>
    </nav>
@endsection
    ...
    <script>
        $(document).ready(function () {
            ...
            var dTable = $('#table').DataTable({
                ...
                ajax: {
                    url: "{{route('admin.salaryReport.index')}}",
                    type: "get"
                },
                columns: [
                    {data: "DT_RowIndex", name: "DT_RowIndex", orderable: false, searchable: false},
                    {data: 'employee_id', name: 'employee_id', orderable: true, searchable: true},
                    {data: 'current_salary', name: 'current_salary', orderable: true, searchable: true},
        ...
        function deleteItem(id) {
            var url = '{{ route("admin.salaryReport.destroy",":id") }}';
            $.ajax({
                type: "DELETE",
        ...
        function statusChange(id) {
            var url = '{{ route("admin.salaryReport.update.status",":id") }}';
            $.ajax({
                type: "GET",
                url: url.replace(':id', id),
                success: function (resp) {
