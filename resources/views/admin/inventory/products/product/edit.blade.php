...
@section('breadcrumb')
    <nav aria-label="breadcrumb" class="d-flex align-items-center justify-content-between" style="width: 100%">
        <ol class="breadcrumb my-0 ms-2">
            ...
            <li class="breadcrumb-item">
                 <a href="{{route('admin.inventory.product.index')}}">Product Edit </a>
            </li>
        </ol>
...
@section('content')
    ...
    <form action="{{ route('admin.inventory.product.update',$product->id) }}" enctype="multipart/form-data" method="POST">
        @csrf
        @method('PUT')
        <div class="row">
            <div class="col-md-12">
                    ...
                    <div class="card-body">
                        <div class="row">
                                ...
                                <div class="form-group col-12 col-sm-12 col-md-6 mb-2">
                                    ...
                                    <div class="input-group mb-3">
                                        ...
                                        <input type="text" name="product_code" id="product_code"class="form-control @error('product_code') is-invalid @enderror"value="{{$product->code}}"placeholder="Enter Product Category Code">
                                      </div>
                                    @error('product_code')
                                        <span class="alert text-danger" role="alert">
                                            <strong>{{ $message }}</strong>
                                ...
                                <div class="form-group col-12 col-sm-12 col-md-4 mb-2">
                                    ...
                                    <select name="sale_unit_id" id="sale_unit_id"class="form-select @error('sale_unit_id') is-invalid @enderror">
                                            <option value="" selected>--Select Category--</option>
                                            @forelse ($units as $item)
                                                <option value="{{ $item->id }}" {{$item->id ==  $product->sale_unit_id ? 'selected' : ''  }}>
                                                    {{ $item->name }}
                                                </option>
                                            @empty
                                                <option value="">No Unit</option>
                                ...
                                <div class="form-group col-12 col-sm-12 col-md-4 mb-2">
                                    ...
                                    <select name="purchase_unit_id" id="purchase_unit_id"class="form-select @error('purchase_unit_id') is-invalid @enderror">
                                            <option value="" selected>--Select Category--</option>
                                            @forelse ($units as $item)
                                                <option value="{{ $item->id }}" {{$item->id ==  $product->purchase_unit_id ? 'selected' : ''  }}>
                                                    {{ $item->name }}
                                                </option>
                                            @empty
                                                <option value="">No Unit</option>
                                ...
                                <div class="form-group col-12 col-sm-12 col-md-4 mb-2">
                                    ...
                                    <input type="number" name="alert_quantity" value="{{$product->alert_quantity}}" class="form-control" step="any">
                                ...
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        <select name="tax_method" class="form-control selectpicker">
                                            ...
                                            <option value="1" {{$product->tax_method == 1 ? 'selected' : ''}}>Exclusive</option>
                                            <option value="2" {{$product->tax_method == 2 ? 'selected' : ''}}>Inclusive</option>
                                        </select>
                                    </div>
                                </div>
                        </div>
                        <div class="form-group col-12 col-sm-12 col-md-6 mb-2">
                            ...
                            <div class="col-md-12 mt-3" id="variant-option">
                                <h5><input name="is_variant" type="checkbox" id="is-variant" value="1" {{$product->is_variant ? 'checked' : ''}}>This product has variant</h5>
                            </div>
                            <div class="row">
                                <div id="variant-section">
                                    @foreach ($productVarient as $key=>$item)
                                        <div class="form-group col-12 col-sm-12 col-md-6 mb-2">
                                            <label for="option"><b>Variant Name</b></label>
                                            <input type="text" name="option[]" class="form-control" value="{{$item->varients->name}}" placeholder="Enter Vairant Nasme ">
                                        </div>
                                        <div class="form-group col-12 col-sm-12 col-md-6 mb-2">
                                            <label for="option"><b>Value</b></label>
                                            <input type="text" name="value[]" class="form-control" value="{{$item->value}}" placeholder="Enter variant Value ">
                                        </div>
                                        <div class="form-group col-12 col-sm-12 col-md-6 mb-2">
                                            <label for="option"><b>Additional Cost</b></label>
                                            <input type="number" name="additional_cost[]" class="form-control" value="{{$item->additional_cost}}" placeholder="Enter Additional Cost " step="any">
                                        </div>
                                        <div class="form-group col-12 col-sm-12 col-md-6 mb-2">
                                            <label for="option"><b>Additional Cost</b></label>
                                            <input type="number" name="additional_price[]" class="form-control" value="{{$item->additional_price}}" placeholder="Enter Additional Price " step="any">
                                        </div>
                                    @endforeach
                                    {{-- append Here --}}
                                ...
                                <div class="col-md-12 mt-3" id="batch-option">
                                    <h5><input name="is_batch" type="checkbox" id="is-batch" value="1" {{$product->is_batch? 'checked' : ''}}>This product has batch and expired date</h5>
                                </div>
                                <div class="col-md-12 mt-2" id="diffPrice-option">
                                    <h5><input name="is_diffPrice" type="checkbox" id="is-diffPrice" value="1" {{$product->is_diffPrice? 'checked' : ''}}>This product has different price for different warehouse</h5>
                                </div>
                                            ...
                                            <thead>
                                                ...
                                                @foreach($warehouse_list as $warehouse)
                                                    <tr>
                                                        ...
                                                        <td><input type="number" name="diff_price[]" @foreach ($productWarehouse as $warehousep) @if($warehousep->id == $warehouse->id ) value="{{$warehousep->price}}"  @endif  @endforeach class="form-control"></td>
                                                    </tr>
                                                @endforeach
                                            </thead>
                                            <tbody>
