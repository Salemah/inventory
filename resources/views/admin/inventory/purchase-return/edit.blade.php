...
@section('breadcrumb')
    <nav aria-label="breadcrumb" class="d-flex align-items-center justify-content-between" style="width: 100%">
        <ol class="breadcrumb my-0 ms-2">
            ...
            <li class="breadcrumb-item">
                 <a href="{{route('admin.inventory.purchase-return.index')}}">Purchase Return </a>
            </li>
            <li class="breadcrumb-item">
                 Edit
            </li>
...
@section('content')
    ...
    <form action="{{ route('admin.inventory.purchase-return.update',$return->id) }}" enctype="multipart/form-data" method="POST">
        @csrf
        @method('PUT')
        <div class="row">
            <div class="col-md-12">
                        ...
                        <div class="row">
                                ...
                                <div class="form-group col-12 col-sm-12 col-md-4 mb-2">
                                    ...
                                    <select name="supplier_id" id="supplier_id" class="form-select @error('supplier_id') is-invalid @enderror">
                                        <option selected value="{{$return->supplier_id}}">
                                            ...
                                            <table class="table table-bordered">
                                                ...
                                                <div >
                                                    <tbody id="purchase_product">
                                                        ...
                                                        <tr>
                                                            <th scope="row">{{ $purchase->products->name}}</th>
                                                            <td>{{$purchase->products->code}}</td>
                                                            <td>{{$purchase->warehouse->name}}</td>
                                                            <td><input type="hidden" id="purchase_quantity_{{$purchase->products->id}}" value="{{$purchase->purchase_qty}}" class="form-control"   readonly > <input type="hidden" id="quantityt-{{$purchase->products->id}}" value="{{$purchase->purchase_qty}}">{{$purchase->purchase_qty}}</td>
                                                        </tr>
                                                        @endforeach
                                                    </tbody>
                                                </div>
                                            ...
                                            <table id="myTable" class="table table-hover order-list">
                                                ...
                                                <tbody id="product_data">
                                                    ...
                                                    <tr id="responsive-table-tr">
                                                        ...
                                                        <td><input type="number" id= "qty-{{$key+1}}" oninput="qtyCheck({{$key+1}})" name="qty[]" class="form-control" value="{{$inventory->purchase_return_qty}}"></td>
...
@push('script')
    <script>
        ...
        $('#product').select2({
            ajax: {
                url: '{{route('admin.inventory.purchase-return.product.search')}}',
                dataType: 'json',
                type: "POST",
                            ...
                            console.log(item);
                               if(Array.isArray(item)){
                                if (item[0].variant_id) {
                                    var p =  item[0].variant.value +' '+item[1].name+'-'+ item[0].products.name;
                                    return {
                                    text: p,
                                    value: item[0].products.id,
                                    id: item[0].products.id,
                                    }
                                    }
                                    else{
        ...
        $('#invoice_id').on("change",  function() {
                ...
                //alert(invoiceId);
                // $('#purchase').show();
                var url = '{{ route('admin.inventory.purchase-return.purchase.search', ':id') }}';
                $.ajax({
                    type: 'GET',
                    ...
                    success: function(data) {
                        ...
                        $('#purchase_product').append('<tr>'+
                                                '<th scope="row">' + val + product.products.name+'</th>'+
                                                '<td>'+product.products.code+'</td>'+
                                                '<td>'+product.warehouse.name+'</td>'+
                                                '<td><input type="hidden" id="purchase_quantity_' + product.products.id + '" value="'+product.purchase_qty+'" class="form-control"   readonly >'+product.purchase_qty+'</td>'+
                                                '</tr>');
                        })
                    }
                });
