...
@section('breadcrumb')
    <nav aria-label="breadcrumb" class="d-flex align-items-center justify-content-between" style="width: 100%">
        <ol class="breadcrumb my-0 ms-2">
            ...
            <li class="breadcrumb-item">
                 <a href="{{route('admin.inventory.purchase-return.index')}}">Sale Return </a>
            </li>
            <li class="breadcrumb-item">
                 Edit
            </li>
        </ol>
        <a href="{{ route('admin.inventory.purchase-return.index') }}" class="btn btn-sm btn-dark">Back to list</a>
    </nav>
@endsection
...
@section('content')
    ...
    <form action="{{ route('admin.inventory.sale-return.update',$return->id) }}" enctype="multipart/form-data" method="POST">
        @csrf
        @method('PUT')
        <div class="row">
            <div class="col-md-12">
                    ...
                    <div class="card-body">
                        <div class="row">
                                <div class="form-group col-12 col-sm-12 col-md-6 mb-2">
                                    ...
                                    <input type="date" class="form-control @error('datee') is-invalid @enderror"value="{{$return->date}}" name="date" id="datepicker">
                                    @error('date')
                                    ...
                                    <label for="invoice_id"><b>Invoice</b><span class="text-danger">*</span></label>
                                        <select name="invoice_id" id="invoice_id"class="form-select @error('invoice_id') is-invalid @enderror">
                                            ...
                                            @forelse ($invoices as $item)
                                                <option value="{{ $item->id }}" {{$item->invoice_no == $return->invoice_no ? 'selected' : '' }}>
                                                    {{ $item->invoice_no }}
                                                </option>
                                            @empty
                                ...
                                <div class="form-group col-12 col-sm-12 col-md-6 mb-2">
                                        ...
                                        <select class="form-control" name="order_tax_rate" id="tax_id">
                                            ...
                                            @foreach($taxs as $tax)
                                            <option value="{{$tax->rate}}" {{$return->total_tax == $tax->rate ? 'selected': '' }}>{{$tax->name}}</option>
                                            @endforeach
                                        </select>
                                </div>
                                ...
                                <div class="row mt-4">
                                    <div class="col-md-12">
                                        <div class="table-responsive mt-3" id="purchase" >
                                            <h5>Purchase Data</h5>
                                            <table class="table table-bordered">
                                                ...
                                                <div >
                                                    <tbody id="purchase_product">
                                                        ...
                                                        <tr>
                                                            <th scope="row">{{ $purchase->variant->value}} {{ $purchase->variant->name}}-{{ $purchase->products->name}}</th>
                                                            <td>{{$purchase->products->code}}</td>
                                                            <td>{{$purchase->warehouse->name}}</td>
                                                            <td><input type="hidden" id="variant_id_{{$purchase->products->id}}{{$purchase->variant->id}}" value="{{$purchase->variant->id}}" class="form-control"   readonly > <input type="hidden" id="purchase_quantity_{{$purchase->products->id}}{{$purchase->variant->id}} '" value="{{$purchase->stock_out}}" class="form-control"   readonly >{{$purchase->stock_out}}</td>
                                                        </tr>
                                                        @endforeach
                                                    </tbody>
                                                </div>
                                            ...
                                            <table id="myTable" class="table table-hover order-list">
                                                ...
                                                <tbody id="product_data">
                                                    ...
                                                    <tr id="responsive-table-tr">
                                                        <input type="hidden" class="purchase-unit" name="purchase_unit[]" value="{{$inventory->sale_unit_id}}">
                                                        <td ><input type="hidden" name="product[]" id="product_id-{{$key+1}}" value="{{$inventory->products->id}}" class="form-control" >{{$inventory->products->name}}</td>
                                                        <td>{{$inventory->products->code}}</td>
                                                        <td><input type="number" id= "qty-{{$key+1}}" oninput="qtyCheck({{$key+1}})" name="qty[]" class="form-control" value="{{$inventory->sale_return_qty}}"></td>
                                                        ...
                                                        <td><input type="hidden" name="variant[]"  id="variant-{{$key+1}}" value="{{$inventory->variant->id}}" class="form-control"   readonly ><button type="button"  class="jDeleteRow form-control btn btn-danger btn-icon waves-effect waves-light text-white" onclick="expensesRemove({{$key+1}})">
                                                        &times;
                                                        </button></td>
                                                ...
                                                <tfoot class="tfoot active">
                                                    ...
                                                    <th id="total-qty"> {{$return->total_qty}}</th>
                                                    ...
                                                    <th id="total">{{$return->grand_total}}</th>
                                                    <th><i class="fa-solid fa-trash"></i></th>
                                                </tfoot>
                                            </table>
...
@push('script')
    <script>
            ...
            $.ajax({
                ...
                success: function ( data) {
                    ...
                    $('#product_data').append('<tr id="responsive-table-tr">'+
                        ...
                        '<td ><input type="hidden" name="product[]" id="product_id-' + x + '" value="' +data[0].id+'" class="form-control" >' +data[0].name+'</td>'+
                        '<td>'+data[0].code+'</td>'+
        ...
        function qtyCheck(x){
            ...
            var variantQuantity = $('#variant_id_64').val();
           // alert(variant+productId+variantQuantity);
            if(qty>0){
                if(variant == variantQuantity){
                    var purchaseQuantity = $('#purchase_quantity_64').val();
        ...
        $('#invoice_id').on("change",  function() {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        $('#purchase_product').append('<tr>'+
                                                '<th scope="row">' + val+ varName  + product.products.name+'</th>'+
                                                '<td>'+product.products.code+'</td>'+
                                                '<td>'+product.warehouse.name+'</td>'+
                                                '<td><input type="hidden" id="variant_id_' + product.products.id +varId+ '" value="'+varId+'" class="form-control"   readonly > <input type="hidden" id="purchase_quantity_' + product.products.id +varId+ '" value="'+product.stock_out+'" class="form-control"   readonly >'+product.purchase_qty+'</td>'+
                                                '</tr>');
                        })
                    }
                });
